    ...
    private heapifyDown() {
        ...
        while (this.getLeftChildIndex(index) < this.heap.length) {
            ...
            if (this.getRightChildIndex(index) < this.heap.length &&
                this.heap[this.getRightChildIndex(index)].length < this.heap[smallerChildIndex].length) {
                smallerChildIndex = this.getRightChildIndex(index);
            }
            if (this.heap[index].length <= this.heap[smallerChildIndex].length) break;
...
function computeKShortestStrings(k: number, words: string[]): string[] {
    ...
    {
        minHeap.add(words[i]);
        // console.log('minhead',minHeap.getWords(),minHeap.size() )

        if (minHeap.size() > k) {
            minHeap.remove();
        }
    }
    return minHeap.getWords()
}
